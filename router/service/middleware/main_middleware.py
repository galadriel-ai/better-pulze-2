...
class MainMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        logger.info(
            f"REQUEST RATE LIMITTING INFO: "
            f" {str(rate_limiter.calls)}"
        )
        if (not password or password != PASSWORD) and "v0/conversation" in request.url.path and request.method == "POST":
